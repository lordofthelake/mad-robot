...
import com.madrobot.di.wizard.xml.converters.AbstractReflectionConverter.DuplicateFieldException;
 ...
 * are written as attributes. The converter requires that all the field types (expect the one with the value) are
 * handled by a {@link SingleValueConverter}. The value field is defined using the name of the type that declares the
 * field and the field name itself. Therefore it is possible to define an inherited field as value. It is also possible
...
public class ToAttributedValueConverter implements Converter {
	...
	public Object unmarshal(final HierarchicalStreamReader reader, final UnmarshallingContext context) {
		...
		while (it.hasNext()) {
			...
			final boolean fieldExistsInClass = reflectionProvider.fieldDefinedInClass(fieldName, resultType);
		...
		if (valueField != null) {
			...
			if (fieldName == null || !reflectionProvider.fieldDefinedInClass(fieldName, resultType)) {
