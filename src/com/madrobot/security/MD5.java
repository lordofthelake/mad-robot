 ...
 * <p>
 * Message-Digest algorithm 5 implementation, It is really fast and optimized
...
class MD5 {
	...
	synchronized byte[] Final() {
		...
		return Encode(finals.getState(), 16);
	}
	 ...
	 *            buffer.length)
	 */
	final void Update(final MD5State stat, byte buffer[], int offset, int length) {
		...
		index = (int) (stat.getCount() & 0x3f);
		stat.setCount(state.getCount() + length);// count += length;
